// import * as joi from "joi";
// import * as fs from 'fs';
// import * as path from 'path';
// import {func} from "joi";
const schema = require('./shema-transformTo-table.js');
const _ = require('underscore');



const json_parse = function(json) {
    try {
        return JSON.parse(json);
    } catch (err) {
        return {};
    }
};
// 处理字符串换行
const handleWrap = str => {
    return _.isString(str) ? str.replace(/\n/gi, '<br/>') : str;
};
const messageMap = {
    desc: '备注',
    default: '实例',
    maximum: '最大值',
    minimum: '最小值',
    maxItems: '最大数量',
    minItems: '最小数量',
    maxLength: '最大长度',
    minLength: '最小长度',
    uniqueItems: '元素是否都不同',
    itemType: 'item 类型',
    format: 'format',
    enum: '枚举',
    enumDesc: '枚举备注'
};

const columns = [
    {
        title: '名称',
        dataIndex: 'name',
        key: 'name'
    },
    {
        title: '类型',
        dataIndex: 'type',
        key: 'type'
    },
    {
        title: '是否必须',
        dataIndex: 'required',
        key: 'required'
    },
    {
        title: '默认值',
        dataIndex: 'default',
        key: 'default'
    },
    {
        title: '备注',
        dataIndex: 'desc',
        key: 'desc'
    },
    {
        title: '其他信息',
        dataIndex: 'sub',
        key: 'sub'
    }
];

function escapeStr(str, isToc) {
    return isToc ? escape(str) : str;
}

function createBaseMessage(basepath, inter) {
    // 基本信息
    let headersTable = `= ${inter.title} =\n{| class="wikitable" width="600" 
|-
| 协议 
| http
|-
| 方式
|${inter.method}
|-
| 地址
|${inter.path}
|-
|描述
|${inter.desc==undefined ?"":inter.desc}
|}\n`;
    return headersTable;

}

function createReqHeaders(req_headers) {
    // Request-headers
    if (req_headers && req_headers.length) {
        let headersTable = `== 请求头 ==\n{| class="wikitable" width="600"\n`;
        headersTable += `|-\n| 参数名称  \n| 参数类型  \n|  是否必须 \n| 示例  \n| 备注 \n`;
        for (let j = 0; j < req_headers.length; j++) {
            headersTable += `|-\n| ${req_headers[j].name || ''}\n|  ${req_headers[j].value || ''}\n|${
                req_headers[j].required == 1 ? '是' : '否'}\n|  ${handleWrap(req_headers[j].example) || ''} \n|  ${handleWrap(req_headers[j].desc) ||''}\n`;
        }
        return headersTable+"|}\n\n";
    }
    return '';
}

function createPathParams(req_params) {
    if (req_params && req_params.length) {
        let paramsTable = `== url 请求参数 == \n {| class="wikitable" width="600"\n|-\n`;
        paramsTable += `| 参数名称  \n| 参数类型  \n|  是否必须 \n| 示例  \n| 备注  \n |}`;
        for (let j = 0; j < req_params.length; j++) {
            paramsTable += `|-\n| ${req_params[j].name || ''} |  ${handleWrap(req_params[j].example) ||''} |  ${handleWrap(req_params[j].desc) || ''}\n`;
        }
        return paramsTable;
    }
    return '';
}

function createReqQuery(req_query) {
    if (req_query && req_query.length) {
        let headersTable = `== get 请求参数 == \n{| class="wikitable" width="600"\n|-\n`;
        headersTable +=`| 参数名称  \n| 是否必须 \n| 示例  \n| 备注  \n`;
        for (let j = 0; j < req_query.length; j++) {
            headersTable += `|-\n| ${req_query[j].name || ''} \n| ${req_query[j].required == 1 ? '是' : '否'}  \n|  ${handleWrap(req_query[j].example) || ''} \n|  ${handleWrap(req_query[j].desc) ||''} \n`;
        }
        return headersTable+"|}\n";
    }
    return '';
}

function createReqBody(req_body_type, req_body_form, req_body_other, req_body_is_json_schema) {
    if (req_body_type === 'form' && req_body_form.length) {
        let bodyTable = `== post 请求参数 == \n{| class="wikitable" width="600"\n`;
        bodyTable += `| 参数名称  \n| 参数类型  \n|  是否必须 \n| 示例  \n| 备注  \n `;
        let req_body = req_body_form;
        for (let j = 0; j < req_body.length; j++) {
            bodyTable += `|-\n| ${req_body[j].name || ''} | ${req_body[j].type || ''}  |  ${req_body[j].required == 1 ? '是' : '否'} \n|  ${req_body[j].example || ''}  \n|  ${req_body[j].desc || ''} \n`;
        }
        return `${bodyTable}\n\n`;
    } else if (req_body_other) {
        if (req_body_is_json_schema) {
            let reqBody = createSchemaTable(req_body_other);
            return `== post 请求参数 == \n` + reqBody;
        } else {
            //other
            return `== post 请求参数 ==\n${req_body_other || ''}` + '\n```';
        }
    }
    return '';
}

function tableHeader(columns) {
    let header = ``;
    columns.map(item => {
        header += `| ${item.key}:${item.title}\n`;
    });

    return header;
}

function handleObject(text) {
    if (!_.isObject(text)) {
        return text;
    }
    let tpl = ``;
    Object.keys(text || {}).map((item, index) => {
        let name = messageMap[item];
        let value = text[item];
        tpl += _.isUndefined(text[item])
            ? ''
            : `<p key=${index}><span style="font-weight: '700'">${name}: </span><span>${value.toString()}</span></p>`;
    });

    return tpl;
}

function tableCol(col, columns, level) {
    let tpl = ``;
    columns.map((item, index) => {
        let dataIndex = item.dataIndex;
        let value = col[dataIndex];
        value = _.isUndefined(value) ? '' : value;
        let text = ``;

        switch (dataIndex) {
            case 'sub':
                text = handleObject(value);
                break;
            case 'type':
                text =
                    value === 'array'
                        ? ` ${col.sub ? col.sub.itemType || '' : 'array'} []`
                        : `${value}`;
                break;
            case 'required':
                text = value ? '必须' : '非必须';
                break;
            case 'desc':
                text = _.isUndefined(col.childrenDesc) ? `${value}`: `${col.childrenDesc}`;
                break;
            case 'name':
                text = `${value}`;
                break;
            default:
                // text = value;
                tpl = tpl.substr(0,tpl.length-2);
        }

        tpl += `| ${text}\n`;
    });
    return `|-\n`+tpl;
}

function tableBody(dataSource, columns, level) {
    //  按照columns的顺序排列数据
    let tpl = ``;
    dataSource.map(col => {
        let child = null;
        tpl += `${tableCol(col, columns, level)}`;
        if (!_.isUndefined(col.children) && _.isArray(col.children)) {
            let index = level + 1;
            child = tableBody(col.children, columns, index) ;
        }
        tpl += child ? `${child}` : ``;
    });
    return tpl ;
}

function createSchemaTable(body) {
    let template = ``;
    let dataSource = schema.schemaTransformToTable(json_parse(body));
    template += `{| class="wikitable" width="600" \n`+ `|-\n| 参数名称  \n| 参数类型  \n|  是否必须 \n| 示例  \n| 备注 \n`;

    template += `${tableBody(dataSource, columns, 0)} \n `;

    return template+`|}`;
}

function createResponse(res_body, res_body_is_json_schema, res_body_type) {
    let resTitle = `\n== 响应参数 ==\n`;
    if (res_body) {
        if (res_body_is_json_schema && res_body_type === 'json') {
            let resBody = createSchemaTable(res_body);
            return resTitle + resBody;
        } else {
            let resBody = '```javascript' + `\n${res_body || ''}\n` + '```';
            return resTitle + resBody;
        }
    }
    return '';
}

function createInterMarkdown(basepath, listItem, isToc) {
    let mdTemplate = ``;
    // 基本信息
    mdTemplate += createBaseMessage(basepath, listItem);
    // Request-headers
    mdTemplate += createReqHeaders(listItem.req_headers);
    // Request-params
    mdTemplate += createPathParams(listItem.req_params);
    // Request-query
    mdTemplate += createReqQuery(listItem.req_query);
    // Request-body
    mdTemplate += createReqBody(
        listItem.req_body_type,
        listItem.req_body_form,
        listItem.req_body_other,
        listItem.req_body_is_json_schema
    );
    // Response
    // Response-body
    mdTemplate += createResponse(
        listItem.res_body,
        listItem.res_body_is_json_schema,
        listItem.res_body_type
    );

    return mdTemplate;
}

function createProjectMarkdown(curProject, wikiData) {
    let mdTemplate = ``;
    // 项目名、项目描述
    let title = `<h1 class="curproject-name"> ${curProject.name} </h1>`;

    mdTemplate += `\n ${title} \n ${curProject.desc || ''}\n\n`;

    // 增加公共wiki信息展示
    mdTemplate += wikiData ? `\n### 公共信息\n${wikiData.desc || ''}\n` : '';
    return mdTemplate;
}

function gen_api_wiki(api_body){
    let wiki = '';
    for(let project of api_body){
        for(let api of project.list){
            wiki += createInterMarkdown(api.query_path,api,false)
        }
    }
    return wiki;
}

export  default {
    gen_api_wiki:gen_api_wiki
}

// let r = {
//     createInterMarkdown,
//     createProjectMarkdown,
//     gen_api_wiki,
// };
//
// module.exports = r;


// function  main() {
//     let fileName = process.argv[1];
//     console.log(fileName)
//     let api_json = fs.readFileSync(fileName).toString();
//     let api_body = JSON.parse(api_json);
//     console.log(gen_api_wiki(api_body));
// }
//
// main();
